...
from public.utils import escape_xss
...
def friend_link_create_signal(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        title = f'增加一个新的{escape_xss(instance._meta.verbose_name)}:{escape_xss(instance.name)}'
        admin_url = reverse('admin:blog_friendlink_change', args=[instance.id])
        content = f'<p><a href="{admin_url}">友链地址：{escape_xss(instance.link)}，' \
                  f'描述：{escape_xss(instance.description)}，待管理员审核！！！</a></p>'
