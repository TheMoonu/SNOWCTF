...
class Subject(models.Model):
    ...
    #is_vulner = models.BooleanField('是否为漏洞库', default=False)
    class Meta:
        verbose_name = '专题'
        verbose_name_plural = verbose_name
        ordering = ['sort_order']
    ...
    def get_article_count(self):
        ...
        for each in self.get_topics():
            num += each.get_articles_data().count()
        return num
...
class Article(models.Model):
    ...
    img_link = ProcessedImageField(upload_to='article/upload/%Y/%m/%d/',
                                   ...
                                   blank=True,
                                   null=True,
                                   help_text='上传图片大小建议使用5:5的宽高比，为了清晰度原始图片宽度应该超过480px'
                                   )
    create_date = models.DateTimeField(verbose_name='创建时间', auto_now_add=True)
    update_date = models.DateTimeField(verbose_name='修改时间', auto_now=True)
    ...
    is_memberShow = models.BooleanField('是否是会员文章', default=False)
    #
    ...
    def img_height(self):
        try:
            return self.img_link.height if self.img_link else 0
        except (FileNotFoundError, OSError, ValueError):
            return 240  # 返回默认高度


    
class vulnerabilitywiki(models.Model):
    def top_10_newest(self):
        return Article.objects.filter(is_publish=True)

    def top_10_most_viewed(self):
        return Article.objects.filter(is_publish=True).order_by('-views')[:10]
    
    def top_10_most_author(self):
        return Article.objects.filter(is_publish=True).order_by('-author')[:10]
...
class PrivacyBlog(models.Model):
