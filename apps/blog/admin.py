...
class ArticleAdmin(admin.ModelAdmin):
    ...
    list_display = ('id', 'title', 'author', 'is_top',
                    'is_publish','is_memberShow', 'topic','topic_order', 'topic_short_title')

    # 字段归类显示
    fieldsets = (
        ('文章信息', {'fields': (('title', 'slug'), 'summary', 'body', 'img_link',
                             ('is_top', 'is_publish','is_memberShow'))}),
        ('文章关系信息', {'fields': ('author', 'category', 'tags', 'keywords')}),
        ('文章专题信息', {'fields': (('topic', 'topic_order'), 'topic_short_title')}),
    )

    # 允许直接编辑的字段，对于布尔值的字段，这个非常有用
    list_editable = ('is_top', 'is_publish','is_memberShow','topic','topic_order', 'topic_short_title')

    # 设置需要添加<a>标签的字段
    list_display_links = ('title',)

    # 激活过滤器，这个很有用
    list_filter = ('create_date', 'category', 'is_top', 'is_publish','is_memberShow', 'tags','topic', 'topic_order')
    ...
    actions = [
        ...
        'make_vulnerability', 'make_not_vulnerability',
        'make_member_show', 'make_not_member_show'
    ]
    
    def make_published(self, request, queryset):
        queryset.update(is_publish=True)
    make_published.short_description = "设为发布"
    
    def make_unpublished(self, request, queryset):
        queryset.update(is_publish=False)
    make_unpublished.short_description = "设为不发布"
    
    def make_member_show(self, request, queryset):
        queryset.update(is_memberShow=True)
    make_member_show.short_description = "设为会员文章"
    
    def make_not_member_show(self, request, queryset):
        queryset.update(is_memberShow=False)
    make_not_member_show.short_description = "设为非会员文章"
...
@admin.register(PrivacyBlog)
class PrivacyBlogAdmin(admin.ModelAdmin):
    list_display = ('short_body', 'create_date', 'update_date')

    def short_body(self, obj):
        return '自由编辑 About 页面的内容，支持 markdown 语法。'

    short_body.short_description = 'PrivacyBlog'

    # 限制用户权限，只能超管可以编辑
    def get_queryset(self, request):
        qs = super(PrivacyBlogAdmin, self).get_queryset(request)
        if request.user.is_superuser:
            return qs
        return None
