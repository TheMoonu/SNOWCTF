...
# 使用apps.blog作为logger名称，匹配settings.py中的配置
...
def get_today_views_by_forecast():
    ...
    if all([last_week_article_hours.get('23'), last_week_page_hours.get('23'),
        ...
        if (thi_done_views < yes_total_views or last_week_done_views < last_week_yes_total_views):
                return ('-', '-')

            # 检查除数不为零
        denominator = (last_week_done_views - last_week_yes_total_views)
        if denominator == 0:
            return ('-', '-')
