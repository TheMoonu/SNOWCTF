...
from .views import (IndexView, DetailView, CategoryView, TagView, AboutView,
                    SilianView, MySearchView, WikiSearchView, ArchiveView, TimelineView, DetailEditView,
                    update_article, FriendLinkView, friend_add, SubjectDetailView,
                    SubjectPageDetailView, SubjectListView, dashboard, feed_hub,vulnerability_articles_view,index,PrivacyView)

urlpatterns = [
    path('', IndexView.as_view(), name='index'),  # Wiki 首页
    #path('', IndexView.as_view(), name='blog_list'),  # 博客列表页（保留）
    ...
    path('privacy/', PrivacyView, name='privacy'),  # 隐私保护页面
    ...
    path('search/', MySearchView.as_view(), name='search_view'),  # 全文搜索 # 全文搜索
    path('wiki-search/', WikiSearchView.as_view(), name='wiki_search'),  # Wiki 文章搜索
    ...
    path('vulnerability-wiki/', vulnerability_articles_view.as_view(), name='vulnerabilitywiki'),
    #path('ctf/', ctfer_view.as_view(), name='ctf'),

    # 专题列表页
    #path('subject/', SubjectListView.as_view(), name='subject_index'),
    path('subject/',index, name='subject_index'),
    # 专题详情页
