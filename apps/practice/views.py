...
def create_web_container(request):
    ...
    try:
        ...
        try:
            ...
            challenge = PC_Challenge.objects.select_related('docker_image', 'network_topology_config').get(uuid=challenge_uuid)
            ...
            if challenge.docker_image or (hasattr(challenge, 'network_topology_config') and challenge.network_topology_config):
                ...
                if not success:
                    ...
                    logger.warning(
                        f"资源预检失败: user={user.id}, challenge={challenge_uuid}, "
                        f"错误={error_msg}"
                    )
                    return JsonResponse({
                        "error": error_msg,
            ...
            cache.delete(container_lock_key)
            logger.error(f"资源预检异常: {str(e)}", exc_info=True)
