    ...
    function destroyContainer() {
        ...
        return $.ajax({
            ...
            success: function(data) {
                ...
                if (typeof showTechSuccess === 'function') {
                    showTechSuccess("容器已自动销毁");
                }
            ...
            error: function(xhr, status, error) {
                ...
                if (typeof showTechError === 'function') {
                    showTechError("容器销毁失败，请刷新页面重试");
                }
                isRequestPending = false;
            },
            complete: function() {
    ...
    function pollTaskStatus(taskId, competitionSlug) {
        ...
        function doPoll() {
            ...
            $.ajax({
                ...
                dataType: 'json',
                showLoading: false,  // 禁用加载动画（轮询过程中使用进度条）
                    ...
                    if (state === 'PENDING' || state === 'PROGRESS') {
                        ...
                        setTimeout(function() {
                            if (result && result.container_urls && result.expires_at) {
                                ...
                                if (typeof showTechSuccess === 'function') {
                                    showTechSuccess("容器创建成功");
                                }
                            } else {
                                console.error('返回数据格式错误：', result);
                                showCreateButton();
                                if (typeof showTechError === 'function') {
                                    showTechError('容器创建成功但信息格式异常，请刷新页面');
                                }
                            }
                            currentTaskId = null;
                        }, 500);  
                        ...
                        showCreateButton();
                        if (typeof showTechWarning === 'function') {
                            showTechWarning("容器创建已取消");
                        }
                    }
                },
                error: function(xhr, status, error) {
                        ...
                        $.ajax({
                            ...
                            error: function(xhr) {
                                ...
                                if (xhr.responseJSON && xhr.responseJSON.error) {
                                    showTechError(xhr.responseJSON.error);
                                }
                            }
                        });
                    }
